...
def film_admin():
    """
    Вывод всей информации обо всех фильмах
    :return:
...
def search_year():
    ...
    form = SearchYearForm()
    if form.validate_on_submit():
        # получить все фильмы определенного года
        ...
        return render_template('film_user.html', username=session['username'], title='Просмотр базы', films=films)
    return render_template("search_year.html", title='Подбор по году', form=form)


@app.route('/search_cinema', methods=['GET', 'POST'])
def search_cinema():
    """
    Запрос фильмов, существующих в определенном кинотеатре
    """
