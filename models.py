...
class CinemasModel:
    ...
    def get(self, cinema_id):
        ...
        cursor = self.connection.cursor()
        cursor.execute("SELECT * FROM cinemas WHERE cinema_id = ?", (str(cinema_id)))
        row = cursor.fetchone()
        return row
...
class FilmsModel:
    ...
    def insert(self, name, year, rating, country, cinema):
        """Добавление фильма"""
        cursor = self.connection.cursor()
        cursor.execute('''INSERT INTO films 
                          (name, year, rating, country, cinema) 
                          VALUES (?,?,?,?,?)''',
                       (name, str(year), str(rating), country, str(cinema)))
        cursor.close()
        self.connection.commit()
    ...
    def get_by_year(self, start_year, end_year):
        ...
        cursor = self.connection.cursor()
        cursor.execute("SELECT name, year, film_id FROM films WHERE year >= ? AND year <= ?", (str(start_year), str(end_year)))
        row = cursor.fetchall()
        return row

    def get_by_cinema(self, cinema_id):
        """Запрос фильмов по кинотеатру"""
        cursor = self.connection.cursor()
